...
def load_data(num,covert_or_overt,elecs,percenta,band):
    ...
    cut_slide=sample_percentages(ECoG[i].shape[0]-1)
    real_perce=cut_slide[percenta]
    for i in range(num_all):
        labels.append(i*np.ones(ECoG[i].shape[0])[real_perce])
        all_erps.append(ECoG[i][real_perce])
